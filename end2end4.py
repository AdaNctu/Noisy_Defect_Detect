...
from PAL_models import QAM_OCM
import numpy as np
import os
...
class End2End4:
    ...
    def train(self):
        ...
        qam_model = QAM_OCM(self.cfg.INPUT_CHANNELS+1).to(device)
        optimizer = self._get_optimizer(model)
        qam_optimizer = torch.optim.SGD(qam_model.parameters(), 1e-4)
        ...
        train_results = self._train_model(device, model, qam_model, train_loader, loss_seg, optimizer, qam_optimizer, validation_loader, tensorboard_writer)
    ...
    def training_iteration_qam(self, data, device, model, qam_model, criterion_seg, optimizer, tensorboard_writer, iter_index):
        ...
        with torch.no_grad():
            _, output_seg_mask = model(images, seg_masks)
            loss_seg = torch.mean(criterion_seg(output_seg_mask, train_masks) * seg_loss_masks, dim=(1,2,3))
        
        loss_weight = qam_model(images, seg_masks)
        loss = torch.sum(loss_weight*loss_seg)
    ...
    def training_iteration(self, data, device, model, qam_model, criterion_seg, optimizer, tensorboard_writer, iter_index):
        images, seg_masks, seg_loss_masks, is_segmented, _, is_pos, train_masks, _ = data
        ...
        total_loss = 0
        with torch.no_grad():
            loss_weight = qam_model(images.to(device), seg_masks.to(device))
        ...
        for sub_iter in range(num_subiters):
            ...
            loss_weight_ = loss_weight[sub_iter * memory_fit:(sub_iter + 1) * memory_fit].to(device)
    ...
    def _train_model(self, device, model, qam_model, train_loader, criterion_seg, optimizer, qam_optimizer, validation_set, tensorboard_writer):
        losses = []
        validation_data = []
        max_validation = -1
        ...
        for epoch in range(num_epochs):
            ...
            if epoch == num_epochs - num_epochs//4:
                for g in optimizer.param_groups:
                    g['lr'] = g['lr']*0.1
                for g in qam_optimizer.param_groups:
                    g['lr'] = g['lr']*0.1
            ...
            for iter_index, (data) in enumerate(train_loader):
                ...
                model.eval()
                qam_model.train()
                result = self.training_iteration_qam(
                                                data, device, model, qam_model,
                                                criterion_seg,
                                                qam_optimizer,
                                                tensorboard_writer, 
            ...
            qam_loss = qam_loss / batch_per_epoch
            losses.append((epoch_loss, epoch))

            self._log(
                f"Epoch {epoch + 1}/{num_epochs} ==> avg_loss={epoch_loss:.5f}, avg_qam_loss={qam_loss:.5f} in {end - start:.2f}s/epoch (fwd/bck in {time_acc:.2f}s/epoch)")
